...
import {
    ...
    generateClientConnectLinkAction,
...
interface SocialState {
    ...
    isClientConnectOpen: boolean;
    clientConnectToken: ClientConnectToken | null;
    stats: {
        totalPosts: number;
    ...
    actions: {
        ...
        closeClientConnect: () => void;
        generateClientConnectLink: (clientName: string) => Promise<string | null>;
    };
}
...
const initialComposerState: ComposerState = {
    ...
    settings: {
        ...
        commentToLead: {
            ...
            dmMessage: "Hey {name}! Here is the direct resource link you requested:",
            resourceUrl: "",
            autoCreateContact: true,
        },
        evergreen: {
            enabled: false,
            recycleIntervalDays: 14,
            maxRecycles: 3,
            recyclesCount: 0,
            aiVariation: true,
        },
...
export const useSocialStore = create<SocialState>((set, get) => ({
    ...
    isClientConnectOpen: false,
    clientConnectToken: null,
    stats: {
        totalPosts: 0,
        scheduledCount: 0,
        publishedCount: 0,
    ...
    actions: {
        ...
        openClientConnect: () => set({ isClientConnectOpen: true }),
        closeClientConnect: () => set({ isClientConnectOpen: false }),

        generateClientConnectLink: async (clientName: string) => {
            try {
                const res = await generateClientConnectLinkAction(clientName);
                if (res.success && res.tokenData) {
                    set({ clientConnectToken: res.tokenData, isClientConnectOpen: true });
                    toast.success("Magic connect link generated!");
                    return res.tokenData.token;
                } else {
                    toast.error(res.error || "Failed to generate link");
                    return null;
                }
            } catch (err) {
                toast.error("Failed to create client onboarding link");
                return null;
            }
        },
    },
