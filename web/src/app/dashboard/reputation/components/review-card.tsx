            ...
            <CardContent className="p-6">
                <div className="flex items-start gap-5">
                    ...
                    <div className="flex-1 min-w-0">
                        ...
                        {/* Reply Section */}
                        {review.reply ? (
                            <div className="mt-4 p-5 rounded-2xl bg-brand-50/40 dark:bg-brand-500/[0.03] border border-brand-100/50 dark:border-brand-500/10">
                                <div className="flex items-center justify-between mb-3">
                                    <div className="flex items-center gap-2 text-xs font-bold text-brand-600 dark:text-brand-400 uppercase tracking-tight">
                            ...
                            !isReplying ? (
                                <div className="flex items-center gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                                    <Button
                                        variant="ghost"
                                        size="sm"
                                        className="text-brand-600 dark:text-brand-400 hover:text-brand-700 hover:bg-brand-50 dark:hover:bg-brand-500/10 h-8 px-3 rounded-lg text-xs font-bold uppercase tracking-wider"
                                ...
                                <div className="mt-4 space-y-3 animate-in fade-in slide-in-from-top-2 duration-300">
                                    <div className="relative group/reply">
                                        <Textarea
                                            ...
                                            className="min-h-[120px] bg-zinc-50 dark:bg-zinc-800/40 border-zinc-200 dark:border-white/[0.08] resize-none focus-visible:ring-brand-500 rounded-2xl p-4 text-sm"
                                            value={replyText}
                                        ...
                                        <div className="absolute top-2 right-2 flex gap-1">
                                            <Button
                                                size="icon"
                                                variant="ghost"
                                                className="h-8 w-8 rounded-full text-zinc-400 hover:text-brand-500"
                                                onClick={() => setShowTemplates(!showTemplates)}
                                                title="Use template"
                                            >
                                            ...
                                            >
                                                {isGeneratingAi ? (
                                                    <div className="w-4 h-4 border-2 border-zinc-300 border-t-brand-500 rounded-full animate-spin" />
                                                ) : (
                                    ...
                                    <div className="flex justify-end gap-2">
                                        ...
                                        <Button
                                            size="sm"
                                            className="bg-brand-600 hover:bg-brand-700 gap-2 h-9 px-5 rounded-xl shadow-lg shadow-brand-500/20 text-xs font-bold uppercase tracking-wider"
                                            onClick={handleReply}
                                ...
                                <div className="space-y-2">
                                    {savedReplies.map((tmpl, idx) => (
                                        <button
                                            ...
                                            className="w-full text-left p-3 rounded-xl bg-white dark:bg-zinc-900 border border-zinc-100 dark:border-white/[0.05] text-xs text-zinc-600 dark:text-zinc-400 hover:border-brand-500 hover:text-brand-500 transition-all font-medium"
                                        >
                                            {tmpl}
                                        </button>
                                    ))}
