...
import type {
    ...
    ShortLinkingPreference,
    CommentToLeadSettings,
    EvergreenSettings,
    CarouselSettings,
    CarouselSlide,
...
export const PLATFORM_SPECS: Record<SocialPlatform, PlatformSpec> = {
    ...
    skool: {
        ...
        maxMedia: 10,
        supportedMediaTypes: ["image", "video"],
        placeholder: "Share an update, discussion prompt, or lesson with your Skool community...",
        brandColor: "#F59E0B",
    },
    whop: {
        ...
        maxMedia: 8,
        supportedMediaTypes: ["image", "video"],
        placeholder: "Post announcement or product drop to your Whop community members...",
        brandColor: "#FF5C35",
    },
};
...
// ── Comment-to-Lead / Comment-to-DM Engine ──────────────────────
...
/**
 * Checks an incoming comment against a post's CommentToLead configuration.
 * Performs clean keyword matching (case-insensitive, whole-word or exact)
 * and prepares the automated DM payload and CRM contact payload.
 */
export function processCommentToLead(
    commentText: string,
    config: CommentToLeadSettings | undefined,
    commenter: { name: string; handle: string; platform: SocialPlatform }
): ProcessCommentResult {
    if (!config || !config.enabled || !config.triggerKeyword) {
        return {
            triggered: false,
            triggerKeyword: "",
            dmMessage: "",
            commenter,
            logMessage: "Comment-to-Lead is not enabled on this post.",
        };
    }

    const keyword = config.triggerKeyword.trim().toUpperCase();
    const cleanComment = commentText.trim().toUpperCase();

    // Check if the comment contains the trigger keyword as a distinct token or exact match
    const tokens = cleanComment.replace(/[^A-Z0-9]/g, " ").split(/\s+/).filter(Boolean);
    const hasMatch = tokens.includes(keyword) || cleanComment === keyword;
    ...
    if (!hasMatch) {
        return {
            ...
            dmMessage: "",
            commenter,
            logMessage: `Comment did not match trigger keyword [${keyword}].`,
        };
    }

    // Build personalized DM message
    let finalDm = config.dmMessage
        .replace(/\{name\}/gi, commenter.name)
        .replace(/\{handle\}/gi, commenter.handle);

    if (config.resourceUrl) {
        finalDm += `\n\nDirect access link: ${config.resourceUrl}`;
    }
    ...
    return {
        ...
        commenter,
        logMessage: `Keyword [${keyword}] triggered! Automated DM prepared for @${commenter.handle}.`,
    };
}

// ── Evergreen Post Queue Utilities ──────────────────────────────
/**
 * Calculates the next schedule timestamp for an evergreen post.
 * Adds intervalDays to the reference date and aligns with preferred morning slot (09:00).
 */
export function calculateEvergreenNextDate(
    intervalDays: number,
    referenceDate: Date = new Date()
): Date {
    const safeDays = Math.max(1, intervalDays || 14);
...
/**
 * Generates an alternative hook and CTA variation for an evergreen post
 * so social platform algorithms don't penalize duplicate text.
 */
export function generateAiHookVariation(originalContent: string): string {
    const trimmed = originalContent.trim();
    if (!trimmed) return originalContent;

    const paragraphs = trimmed.split(/\n+/);
    const body = paragraphs.slice(1).join("\n\n");

    const hookVariations = [
        "A lot of leaders ask us how to solve this exact problem:",
        "Here is a counter-intuitive principle most teams overlook:",
        "If you want to 10x your conversion speed, start here:",
        "The fastest-growing companies do this differently every day:",
        "A quick reminder for anyone working to scale their pipeline:",
    ];
...
// ── LinkedIn Carousel / Slide Builder Configuration ─────────────
export const CAROUSEL_THEMES = [
    {
        id: "dark" as const,
        name: "Modern Dark",
        bgClass: "from-zinc-950 via-zinc-900 to-black text-white",
        accentClass: "text-brand-400 border-brand-500",
    },
    {
        id: "blue" as const,
        name: "Ocean Royal",
        bgClass: "from-blue-950 via-indigo-900 to-slate-950 text-white",
        accentClass: "text-cyan-400 border-cyan-400",
    },
    {
        id: "crimson" as const,
        name: "Sunset Crimson",
        bgClass: "from-rose-950 via-red-900 to-zinc-950 text-white",
        accentClass: "text-amber-400 border-amber-400",
    },
    {
        id: "emerald" as const,
        name: "Forest Emerald",
        bgClass: "from-emerald-950 via-teal-900 to-zinc-950 text-white",
