...
test("Social Studio (Postiz Engine) unit tests", async (t) => {
    await t.test("Platform specs include required social networks and streaming channels", () => {
        ...
        // Skool & Whop (Postiz 2026 flagship community schedulers)
        assert.ok(PLATFORM_SPECS.skool, "Skool spec defined");
        assert.equal(PLATFORM_SPECS.skool.maxCharacters, 10000);
        assert.equal(PLATFORM_SPECS.skool.brandColor, "#F59E0B");

        assert.ok(PLATFORM_SPECS.whop, "Whop spec defined");
        assert.equal(PLATFORM_SPECS.whop.maxCharacters, 5000);
        assert.equal(PLATFORM_SPECS.whop.brandColor, "#FF5C35");
    });
    ...
    await t.test("processCommentToLead matches trigger keywords and personalizes auto-DM", () => {
        const commenter = {
            name: "Sarah Connor",
            handle: "sarahc",
        ...
        // Case 1: Disabled config does not trigger
        const disabledRes = processCommentToLead("Send me the guide!", undefined, commenter);
        assert.equal(disabledRes.triggered, false);

        // Case 2: Matching keyword triggers personalized DM and attaches resource
        const config = {
            enabled: true,
            triggerKeyword: "GUIDE",
            dmMessage: "Hey {name}! Thanks for commenting. Here is your link:",
            resourceUrl: "https://highreach.io/resources/speed-to-lead.pdf",
        };

        const matchingComment = "Love this framework! Please send GUIDE to my inbox.";
        const matchRes = processCommentToLead(matchingComment, config, commenter);
        assert.equal(matchRes.triggered, true);
        assert.equal(matchRes.triggerKeyword, "GUIDE");
        assert.ok(matchRes.dmMessage.includes("Hey Sarah Connor!"));
        assert.ok(matchRes.dmMessage.includes("https://highreach.io/resources/speed-to-lead.pdf"));

        // Case 3: Non-matching comment does not trigger
        const noMatchComment = "Great post, completely agree!";
        const noMatchRes = processCommentToLead(noMatchComment, config, commenter);
        assert.equal(noMatchRes.triggered, false);
    });

    await t.test("calculateEvergreenNextDate advances scheduled time and sets morning queue slot", () => {
        const baseDate = new Date(2026, 2, 10, 15, 0, 0); // March 10, 2026 3pm
        const nextDate = calculateEvergreenNextDate(14, baseDate);

        assert.equal(nextDate.getDate(), 24); // March 24
        assert.equal(nextDate.getMonth(), 2); // March
        assert.equal(nextDate.getHours(), 9); // 9:00 AM slot
        assert.equal(nextDate.getMinutes(), 0);

        // Explicit 1-day interval
        const oneDay = calculateEvergreenNextDate(1, baseDate);
        assert.equal(oneDay.getDate(), 11);

        // Fallback safety for 0 or undefined (defaults to 14 days)
        const fallback = calculateEvergreenNextDate(0, baseDate);
        assert.equal(fallback.getDate(), 24);
    });

    await t.test("generateAiHookVariation crafts dynamic hooks preserving content body", () => {
        const original = "5 Steps to convert more leads\n\n1. Reply in 60 seconds\n2. Automate SMS text-back\n3. Sync to CRM";
        const variation = generateAiHookVariation(original);

        assert.ok(variation.length > 0);
        assert.ok(variation.includes("1. Reply in 60 seconds"));
        assert.ok(variation.includes("3. Sync to CRM"));
        assert.notEqual(variation, original);
    });

    await t.test("CAROUSEL_THEMES includes 4 high-contrast visual themes", () => {
        assert.equal(CAROUSEL_THEMES.length, 4);
        const themeIds = CAROUSEL_THEMES.map(t => t.id);
        assert.ok(themeIds.includes("dark"));
        assert.ok(themeIds.includes("blue"));
