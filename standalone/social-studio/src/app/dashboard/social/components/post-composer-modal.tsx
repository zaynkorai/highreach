...
export function PostComposerModal() {
    ...
    // 2026 HighReach Growth Engines State
    const [isCommentToLeadOpen, setIsCommentToLeadOpen] = useState(false);
    const [isEvergreenOpen, setIsEvergreenOpen] = useState(false);
    const [isCarouselOpen, setIsCarouselOpen] = useState(false);

    const handleToggleCommentToLead = (enabled: boolean) => {
        updateComposer({
            settings: {
                ...composer.settings,
                commentToLead: {
                    enabled,
                    triggerKeyword: composer.settings.commentToLead?.triggerKeyword || "GROWTH",
                    dmMessage: composer.settings.commentToLead?.dmMessage || "Hey {name}! Here is the direct resource link you requested:",
                    resourceUrl: composer.settings.commentToLead?.resourceUrl || "",
                    autoCreateContact: composer.settings.commentToLead?.autoCreateContact ?? true,
                },
            },
        });
    };

    const handleToggleEvergreen = (enabled: boolean) => {
        updateComposer({
            settings: {
                ...composer.settings,
                evergreen: {
                    enabled,
                    recycleIntervalDays: composer.settings.evergreen?.recycleIntervalDays || 14,
                    maxRecycles: composer.settings.evergreen?.maxRecycles || 3,
                    recyclesCount: 0,
                    aiVariation: composer.settings.evergreen?.aiVariation ?? true,
                },
            },
        });
    };
    ...
    const handleToggleCarousel = (enabled: boolean) => {
        const defaultSlides = [
            { id: "1", title: "The High-Converting Hook", body: "90% of outreach campaigns fail within 24 hours. Here is the framework that scales:" },
            { id: "2", title: "The Core Problem", body: "Manual follow-ups create latency. When response times exceed 5 minutes, qualification drop-off exceeds 80%." },
            { id: "3", title: "The 30-Second Solution", body: "Automate speed-to-lead qualification directly into your calendar with zero friction." },
            { id: "4", title: "Action Step", body: "Comment 'SCALE' below to get the full SOP and template completely free." },
        ];
        updateComposer({
            settings: {
                ...composer.settings,
                carousel: {
                    enabled,
                    slides: composer.settings.carousel?.slides?.length ? composer.settings.carousel.slides : defaultSlides,
                    theme: composer.settings.carousel?.theme || "dark",
                },
            },
        });
    };
            ...
            <DialogContent className="sm:max-w-[1500px] w-[96vw] max-h-[92vh] overflow-y-auto p-0 gap-0 rounded-2xl border-zinc-200 dark:border-zinc-800">
                ...
                <div className="grid grid-cols-1 lg:grid-cols-12 min-h-[580px]">
                    ...
                    <div className="lg:col-span-7 p-6 border-b lg:border-b-0 lg:border-r border-zinc-100 dark:border-zinc-800 space-y-6">
                        ...
                        {/* 10. Comment-to-Lead / Comment-to-DM Growth Engine */}
                        <div className="space-y-2 pt-1 border-t border-zinc-100 dark:border-zinc-800/80">
                            <div className="flex items-center justify-between">
                                <button
                                    type="button"
                                    onClick={() => setIsCommentToLeadOpen(!isCommentToLeadOpen)}
                                    className="text-xs font-semibold text-zinc-600 dark:text-zinc-300 flex items-center gap-1.5 hover:text-brand-600"
                                >
                                    <Zap className="w-3.5 h-3.5 text-amber-500" />
                                    <span>Comment-to-DM & Lead Capture</span>
                                    {composer.settings.commentToLead?.enabled && (
                                        <Badge className="bg-amber-500/20 text-amber-600 dark:text-amber-400 text-[10px] font-extrabold px-1.5 py-0 border-0">
                                            ACTIVE [{composer.settings.commentToLead.triggerKeyword}]
                                        </Badge>
                                    )}
                                    <span className="text-[10px] text-zinc-400 font-normal">
                                        {isCommentToLeadOpen ? "(hide)" : "(configure)"}
                                    </span>
                                </button>

                                <button
                                    type="button"
                                    onClick={() => handleToggleCommentToLead(!composer.settings.commentToLead?.enabled)}
                                    className={cn(
                                ...
                                >
                                    {composer.settings.commentToLead?.enabled ? "Enabled" : "Disabled"}
                                </button>
                            </div>

                            {isCommentToLeadOpen && (
                                <div className="p-3.5 rounded-xl bg-amber-50/40 dark:bg-amber-950/20 border border-amber-200/60 dark:border-amber-900/40 space-y-3 text-xs animate-in fade-in duration-200">
                                    <div className="grid grid-cols-2 gap-2">
                                        <div>
                                            ...
                                            <Input
                                                value={composer.settings.commentToLead?.triggerKeyword || ""}
                                                onChange={(e) =>
                                                    updateComposer({
                                                        settings: {
                                                            ...composer.settings,
                                                            commentToLead: {
                                                                ...composer.settings.commentToLead!,
                                                ...
                                                placeholder="GROWTH"
                                                className="h-8 text-xs font-mono font-bold uppercase bg-white dark:bg-zinc-900"
                                            />
                                        </div>
                                        <div>
                                            <Label className="text-[11px] font-semibold text-zinc-600 dark:text-zinc-400">
                                                Resource / Asset URL (optional)
                                            </Label>
                                            <Input
                                                value={composer.settings.commentToLead?.resourceUrl || ""}
                                                onChange={(e) =>
                                                    updateComposer({
                                                        settings: {
                                                            ...composer.settings,
                                                ...
                                                placeholder="https://highreach.io/guide"
                                                className="h-8 text-xs bg-white dark:bg-zinc-900"
                                            />
                                        </div>
                                    </div>
                                    ...
                                    <div>
                                        <Label className="text-[11px] font-semibold text-zinc-600 dark:text-zinc-400">
                                            Automated DM Message (use {"{name}"} for personalization)
                                        </Label>
                                        <Textarea
                                            value={composer.settings.commentToLead?.dmMessage || ""}
                                            onChange={(e) =>
                                                updateComposer({
                                                    settings: {
                                                        ...composer.settings,
                                            ...
                                            rows={2}
                                            placeholder="Hey {name}! Here is the direct link you requested..."
                                            className="text-xs bg-white dark:bg-zinc-900 resize-none"
                                        />
                                    </div>
                                    ...
                                    <label className="flex items-center gap-2 cursor-pointer pt-0.5">
                                        <input
                                            type="checkbox"
                                            checked={composer.settings.commentToLead?.autoCreateContact ?? true}
                                            onChange={(e) =>
                                                updateComposer({
                                                    settings: {
                                                        ...composer.settings,
                                        ...
                                        <span className="text-[11px] font-medium text-zinc-600 dark:text-zinc-300">
                                            Automatically create Contact in HighReach CRM upon keyword trigger
                                        </span>
                                    </label>
                                </div>
                            )}
                        ...
                        {/* 11. Evergreen Queue Recycling */}
                        <div className="space-y-2 pt-1 border-t border-zinc-100 dark:border-zinc-800/80">
                            <div className="flex items-center justify-between">
                                <button
                                    type="button"
                                    onClick={() => setIsEvergreenOpen(!isEvergreenOpen)}
                                    className="text-xs font-semibold text-zinc-600 dark:text-zinc-300 flex items-center gap-1.5 hover:text-brand-600"
                                >
                                    <Repeat2 className="w-3.5 h-3.5 text-emerald-500" />
                                    <span>Evergreen Recycling Queue</span>
                                    {composer.settings.evergreen?.enabled && (
                                        <Badge className="bg-emerald-500/20 text-emerald-600 dark:text-emerald-400 text-[10px] font-extrabold px-1.5 py-0 border-0">
                                            EVERY {composer.settings.evergreen.recycleIntervalDays}D
                                        </Badge>
                                    )}
                                    <span className="text-[10px] text-zinc-400 font-normal">
                                        {isEvergreenOpen ? "(hide)" : "(configure)"}
                                    </span>
                                </button>

                                <button
                                    type="button"
                                    onClick={() => handleToggleEvergreen(!composer.settings.evergreen?.enabled)}
                                    className={cn(
                                        "text-[11px] font-bold px-2 py-0.5 rounded-full border transition-all",
                                        composer.settings.evergreen?.enabled
                                ...
                                >
                                    {composer.settings.evergreen?.enabled ? "Active" : "Inactive"}
                                </button>
                            </div>

                            {isEvergreenOpen && (
                                <div className="p-3.5 rounded-xl bg-emerald-50/40 dark:bg-emerald-950/20 border border-emerald-200/60 dark:border-emerald-900/40 space-y-3 text-xs animate-in fade-in duration-200">
                                    <div className="grid grid-cols-2 gap-2">
                                        <div>
                                            <Label className="text-[11px] font-semibold text-zinc-600 dark:text-zinc-400">
                                                Recycle Every (Days)
                                            ...
                                            <Input
                                                type="number"
                                                min={1}
                                                value={composer.settings.evergreen?.recycleIntervalDays || 14}
                                                onChange={(e) =>
                                                    updateComposer({
                                                        settings: {
                                                            ...composer.settings,
                                                            evergreen: {
                                                                ...composer.settings.evergreen!,
                                                                recycleIntervalDays: parseInt(e.target.value, 10) || 14,
                                                            },
                                                        },
                                                    })
                                                }
                                        ...
                                        <div>
                                            ...
                                            <Input
                                                type="number"
                                                min={0}
                                                value={composer.settings.evergreen?.maxRecycles || 0}
                                                onChange={(e) =>
                                                    updateComposer({
                                                        settings: {
                                                            ...composer.settings,
                                                            evergreen: {
                                                                ...composer.settings.evergreen!,
                                                                maxRecycles: parseInt(e.target.value, 10) || 0,
                                                            },
                                                        },
                                                    })
                                                }
                                    ...
                                    <label className="flex items-center gap-2 cursor-pointer pt-0.5">
                                        <input
                                            type="checkbox"
                                            checked={composer.settings.evergreen?.aiVariation ?? true}
                                            onChange={(e) =>
                                                updateComposer({
                                                    settings: {
                                                        ...composer.settings,
                                        ...
                                        />
                                        <span className="text-[11px] font-medium text-zinc-600 dark:text-zinc-300">
                                            Rewrite hook & CTA with AI on each recycle (avoids algorithm duplication penalties)
                                        </span>
                                    </label>
                                </div>
                            )}
                        ...
                        {/* 12. LinkedIn Document Carousel / PDF Slides Builder */}
                        {composer.selectedPlatforms.includes("linkedin") && (
                            <div className="space-y-2 pt-1 border-t border-zinc-100 dark:border-zinc-800/80">
                                <div className="flex items-center justify-between">
                                    <button
                                        type="button"
                                    ...
                                    >
                                        ...
                                        <span>LinkedIn Document Carousel Maker</span>
                                        {composer.settings.carousel?.enabled && (
                                            <Badge className="bg-blue-500/20 text-blue-600 dark:text-blue-400 text-[10px] font-extrabold px-1.5 py-0 border-0">
                                                {composer.settings.carousel.slides.length} SLIDES
                                            </Badge>
                                        )}
                                        <span className="text-[10px] text-zinc-400 font-normal">
                                            {isCarouselOpen ? "(hide)" : "(edit slides)"}
                                        </span>
                                    </button>

                                    <button
                                        type="button"
                                        onClick={() => handleToggleCarousel(!composer.settings.carousel?.enabled)}
                                        className={cn(
                                            "text-[11px] font-bold px-2 py-0.5 rounded-full border transition-all",
                                            composer.settings.carousel?.enabled
                                    ...
                                    >
                                        {composer.settings.carousel?.enabled ? "Enabled" : "Disabled"}
                                    </button>
                                </div>

                                {isCarouselOpen && (
                                    <div className="p-3.5 rounded-xl bg-blue-50/40 dark:bg-blue-950/20 border border-blue-200/60 dark:border-blue-900/40 space-y-3 text-xs animate-in fade-in duration-200">
                                        <div className="flex items-center justify-between">
                                            <Label className="text-[11px] font-bold text-zinc-600 dark:text-zinc-400">
                                                Visual Color Theme
                                            </Label>
                                            <div className="flex gap-1.5">
                                                {(["dark", "blue", "crimson", "emerald"] as const).map((th) => (
                                                    <button
                                                        key={th}
                                                        type="button"
                                                        onClick={() =>
                                                        ...
                                                        className={cn(
                                                            "px-2 py-0.5 rounded text-[10px] font-bold capitalize border",
                                                            composer.settings.carousel?.theme === th
                                        ...
                                        <div className="space-y-2 max-h-56 overflow-y-auto pr-1">
                                            {composer.settings.carousel?.slides?.map((sl, sIdx) => (
                                                <div key={sl.id || sIdx} className="p-2.5 rounded-lg bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 space-y-1.5">
                                                    <div className="flex items-center justify-between text-[11px]">
                                                        <span className="font-bold text-foreground">Slide #{sIdx + 1}</span>
                                                        <button
                                                            type="button"
                                                            onClick={() => {
                                                                const updatedSlides = composer.settings.carousel!.slides.filter((_, i) => i !== sIdx);
                                                                updateComposer({
                                                                    settings: {
                                                                        ...composer.settings,
                                                                        carousel: { ...composer.settings.carousel!, slides: updatedSlides },
                                                                    },
                                                                });
                                                            }}
                                                            className="text-zinc-400 hover:text-rose-500"
                                                    ...
                                                    <Input
                                                        value={sl.title}
                                                        onChange={(e) => {
                                                            const updated = [...composer.settings.carousel!.slides];
                                                            updated[sIdx] = { ...updated[sIdx], title: e.target.value };
                                                            updateComposer({
                                                                settings: {
                                                                    ...composer.settings,
                                                                    carousel: { ...composer.settings.carousel!, slides: updated },
                                                                },
                                                            });
                                                        }}
                                                    ...
                                                    <Textarea
                                                        ...
                                                        onChange={(e) => {
                                                            const updated = [...composer.settings.carousel!.slides];
                                                            updated[sIdx] = { ...updated[sIdx], body: e.target.value };
                                                            updateComposer({
                                                                settings: {
                                                                    ...composer.settings,
                                                                    carousel: { ...composer.settings.carousel!, slides: updated },
                                                        ...
                                                        rows={2}
                                                        placeholder="Slide explanation, bullet points, or takeaways..."
                                                        className="text-xs resize-none"
                                                    />
                                                </div>
                                            ))}
                                        ...
                                        <Button
                                            ...
                                            variant="outline"
                                            onClick={() => {
                                                const currentSlides = composer.settings.carousel?.slides || [];
                                                const newSlide = {
                                                    id: String(Date.now()),
                                                    title: `Key Insight #${currentSlides.length + 1}`,
                                                    body: "Detail the framework, metrics, or actionable tactics here.",
                                                };
                                                updateComposer({
                                                    settings: {
                                                        ...composer.settings,
                                            ...
                                            className="w-full h-8 text-xs font-semibold gap-1 text-blue-600 hover:bg-blue-50 dark:hover:bg-blue-950/40"
                                        >
                    ...
                    <div className="lg:col-span-5 p-6 bg-zinc-50/50 dark:bg-zinc-950/50 flex flex-col justify-between space-y-4">
                        <div className="space-y-4">
                            ...
                            <PlatformPreview
                                ...
                                carouselSlides={
                                    composer.previewPlatform === "linkedin" && composer.settings.carousel?.enabled
                                        ? composer.settings.carousel.slides
                                        : undefined
                                }
                                carouselTheme={composer.settings.carousel?.theme}
                            />
                        </div>
