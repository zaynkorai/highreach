...
import { db, socialAccounts, socialPosts, socialPostChannels, tenants, contacts } from "@/lib/db";
...
export class SocialService {
    ...
    static async dispatchPublish(tenantId: string, postId: string): Promise<SocialPost> {
        ...
        // ── Evergreen Post Auto-Recycle Hook ────────────────────
        if (finalStatus === "published" && post.settings?.evergreen?.enabled) {
            try {
                const evergreenConfig = post.settings.evergreen;
                const recyclesCount = (evergreenConfig.recyclesCount || 0) + 1;
                const maxRecycles = evergreenConfig.maxRecycles || 0; // 0 = unlimited

                if (maxRecycles === 0 || recyclesCount <= maxRecycles) {
                    const nextScheduledDate = calculateEvergreenNextDate(
                        evergreenConfig.recycleIntervalDays || 14,
                        publishedDate
                    );

                    const nextContent = evergreenConfig.aiVariation
                        ? generateAiHookVariation(post.content)
                        : post.content;

                    await this.createPost(tenantId, post.created_by || undefined, {
                        content: nextContent,
                ...
                console.warn("[Social Studio] Evergreen recycle scheduling error:", evergreenErr);
            }
        }
    ...
    /**
     * Simulates or processes an incoming post comment for Comment-to-Lead conversion.
     * Evaluates trigger keyword, formats automated DM response, and optionally creates
     * a CRM contact inside HighReach.
     */
    static async triggerCommentToLeadSimulation(
        tenantId: string,
        postId: string,
        commenter: {
            name: string;
        ...
        const result = processCommentToLead(
            commenter.commentText,
            post.settings?.commentToLead,
            {
                name: commenter.name,
        ...
        if (result.triggered && post.settings?.commentToLead?.autoCreateContact !== false) {
            try {
                const [newContact] = await db
                    .insert(contacts)
                    .values({
                        tenantId,
                        firstName: commenter.name || commenter.handle,
                        source: "Social Studio (Comment-to-DM)",
                        tags: ["SocialLead", result.triggerKeyword, commenter.platform],
                        notes: `Auto-captured via post "${post.content.slice(0, 40)}...". Triggered keyword [${result.triggerKeyword}]. DM dispatched: "${result.dmMessage.slice(0, 100)}..."`,
                    })
                    .returning();

                createdContactId = newContact.id;
            } catch (contactErr) {
                console.warn("[Social Studio] Failed to auto-create contact from comment:", contactErr);
            }
        }

        return { result, createdContactId };
    }
